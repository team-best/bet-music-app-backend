...
const signUPUser = () => {
  prompt.question('User name:',async function(name) {
    prompt.question('password:',async function(password) {
      let addUserName = await superAgent.post('https://music-guessing-game.herokuapp.com/user').send({username: name, password: password});
...
const guessingGame = () => {
  ...
  prompt.question(chalk.yellow('User name:'), async function(name) {
    ...
    findName= await superAgent.get('https://music-guessing-game.herokuapp.com/user').send({username: name});
  
    console.log('name', name)

    let result = false;
    for(let i = 0; i < findName.body.length; i ++){
      if(findName.body[i].username === name){  
      result = true;
      break;     
      }    
    }
    if(result === false){
      console.log('We couldn\'t find the username in the database')
      guessingGame()
    }else{
...
const guessingGamePassword = async() => {
